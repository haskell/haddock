module Documentation.Markdown.ParserCombinators (
    Position(..)
  , Parser
  , parse
  , (<?>)
...
data ParserState = ParserState { subject  :: String
                               , position :: Position
                               , lastChar :: Maybe Char
                               }
...
advance = L.foldl' go
  where go :: ParserState -> Char -> ParserState
        go st c = st{ subject = drop 1 (subject st)
                    , position = case c of
                                      '\n' -> Position { line =
                                                  line (position st) + 1
                                                  , column = 1 }
...
satisfy f = Parser g
  where g st = case L.uncons (subject st) of
                    Just (c, _) | f c ->
                         success (advance st [c]) c
                    _ -> failure st "character meeting condition"
{-# INLINE satisfy #-}

peekChar :: Parser (Maybe Char)
peekChar = Parser $ \st ->
             case L.uncons (subject st) of
...
takeWhile f = Parser $ \st ->
  let t = L.takeWhile f (subject st) in
...
takeWhile1 f = Parser $ \st ->
  case L.takeWhile f (subject st) of
       t | null t  -> failure st "characters satisfying condition"
...
skip f = Parser $ \st ->
  case L.uncons (subject st) of
       Just (c,_) | f c -> success (advance st (pure c)) ()
