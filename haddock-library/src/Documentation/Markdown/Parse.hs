...
import Documentation.Markdown.Inlines
...
markdown :: String -> Doc
markdown = Doc . processDocument . processLines
  ...
  case top of
       (Container Reference{} cs'') ->
         case parse pReference
               (strip $ joinLines $ map extractText $ toList cs'') of
...
processElts refmap (L _lineNumber lf : rest) =
  case lf of
    ...
    TextLine t -> singleton (Para $ parseInlines refmap txt) <>
               ...
               where txt = stripEnd $ joinLines $ map stripStart
  ...
  case ct of
    ...
    IndentedCode -> singleton (CodeBlock (CodeAttr "" "") txt)
                  ...
                  where txt = joinLines $ stripTrailingEmpties
                        ...
                        -- one more:
                        extractCode (L _ (BlankLine t)) = [drop 1 t]
...
processLines t = (doc, refmap)
  ...
  lns        = zip [1..] (map tabFilter $ lines t)
...
tryOpenContainers :: [Container] -> String -> (String, Int)
...
tryNewContainers :: Bool -> Int -> String -> ([ContainerType], Leaf)
...
textLineOrBlank = consolidate <$> takeText
  where consolidate ts | all (==' ') ts = BlankLine ts
...
leaf lastLineIsText = scanNonindentSpace *> (
     (ATXHeader <$> parseAtxHeaderStart <*>
         (strip . removeATXSuffix <$> takeText))
