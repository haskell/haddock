...
import qualified GHC.Types.Avail as Avail (avail)
...
createInterface1 flags unit_state mod_sum mod_iface ifaces inst_ifaces (instances, fam_instances) = do
  ...
  (docMap :: DocMap Name) <- do
    let docsDecls = Map.fromList $ nonDetUniqMapToList mod_iface_docs.docs_decls
    traverse (processDocStringsParas dflags pkg_name) docsDecls

  (argMap :: Map Name (Map Int (MDoc Name))) <- do
      let docsArgs = Map.fromList $ nonDetUniqMapToList mod_iface_docs.docs_args
  ...
  where
    ...
    --
    listAvail = [ AvailTC listTyConName $ map NormalGreName
                          [listTyConName, nilDataConName, consDataConName] ]
    funAvail  = [ AvailTC fUNTyConName [NormalGreName fUNTyConName] ]
    eqAvail   = [ AvailTC eqTyConName [NormalGreName eqTyConName] ]
    tupsAvail = [ AvailTC tyName [NormalGreName tyName, NormalGreName datName]
                | i<-[0..15]
...
nonDetUniqMapToList (UniqMap.UniqMap m) = nonDetEltsUFM m
