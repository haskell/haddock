        ...
        if use_gadt_syntax
           then return $ noLocA $
              ConDeclGADT { con_g_ext  = []
                          , con_names  = [name]
                          , con_forall = not $ null user_tvbndrs
...
synify_ty_var no_kinds flag tv
  ...
  = noLoc (UserTyVar noAnn flag (noLocA name))
  | otherwise = noLoc (KindedTyVar noAnn flag (noLocA name) (synifyKindSig kind))
  where
    kind = tyVarKind tv
    name = getName tv
...
synifyVisForAllType vs ty =
  ...
  in noLocA $ HsForAllTy { hst_tele    = mkHsForAllVisTele sTvs
                         , hst_xforall = noAnn
