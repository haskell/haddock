...
import GHC.Driver.Monad (Session(..), withTempSession, reflectGhc)
...
createIfaces verbosity modules flags instIfaceMap = do
  ...
  withTempSession installHaddockPlugin $ do
    ...
    case loadOk of
      ...
      Succeeded -> do
        -- Note that getModuleGraph needs to be closed over by withTempSession
  ...
  let
    ...
    processTypeCheckedResult mod_summary tc_gbl_env
      ...
      | otherwise = do
          ...
          liftIO $ atomicModifyIORef' ifaceMapRef $ \xs ->
            (Map.insert (ms_mod mod_summary) iface xs, ())

          liftIO $ atomicModifyIORef' moduleSetRef $ \xs ->
