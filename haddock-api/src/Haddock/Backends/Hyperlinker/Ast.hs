...
variables =
  ...
  where
    var term = case cast term of
        (Just ((GHC.dL->(sspan , GHC.HsVar _ name))
               :: GHC.LHsExpr GHC.GhcRn)) ->
            pure (sspan, RtkVar (GHC.unLoc name))
        (Just (GHC.dL->(_ , GHC.RecordCon _ (GHC.dL->(sspan , name)) _))) ->
            pure (sspan, RtkVar name)
        _ -> empty
    rec term = case cast term of
        Just (GHC.HsRecField (GHC.dL->(sspan , name))
...
types = everythingInRenamedSource ty
  where
    ...
    ty term = case cast term of
        (Just ((GHC.dL->(sspan , GHC.HsTyVar _ _ name))
               :: GHC.LHsType GHC.GhcRn)) ->
            pure (sspan, RtkType (GHC.unLoc name))
        (Just ((GHC.dL->(sspan , GHC.HsOpTy _ l name r))
               :: GHC.LHsType GHC.GhcRn)) ->
...
binds = everythingInRenamedSource
  ...
  where
    fun term = case cast term of
        (Just (GHC.FunBind _ (GHC.dL->(sspan , name)) _ _ _
               :: GHC.HsBind GHC.GhcRn)) ->
            pure (sspan, RtkBind name)
        (Just (GHC.PatSynBind _
               (GHC.PSB _ (GHC.dL->(sspan , name)) args _ _))) ->
            pure (sspan, RtkBind name) ++ everythingInRenamedSource patsyn_binds args
        _ -> empty
    patsyn_binds term = case (cast term) :: Maybe (GHC.Located GHC.Name) of
        Just (GHC.dL->(sspan , name)) -> pure (sspan, RtkVar name)
        _ -> empty
    pat term = case cast term of
        (Just ((GHC.dL->(sspan , GHC.VarPat _ name)) :: GHC.LPat GHC.GhcRn)) ->
            pure (sspan, RtkBind (GHC.unLoc name))
        (Just (GHC.dL->(_ , GHC.ConPatIn (GHC.dL->(sspan , name)) recs))) ->
            [(sspan, RtkVar name)] ++ everythingInRenamedSource rec recs
        (Just (GHC.dL->(_ , GHC.AsPat _ (GHC.dL->(sspan , name)) _))) ->
            pure (sspan, RtkBind name)
        _ -> empty
    rec term = case cast term of
        (Just (GHC.HsRecField (GHC.dL->(sspan , name))
    ...
    tvar term = case cast term of
        (Just ((GHC.dL->(sspan , GHC.UserTyVar _ name))
               :: GHC.LHsTyVarBndr GHC.GhcRn)) ->
            pure (sspan, RtkBind (GHC.unLoc name))
        (Just (GHC.dL->(_ , GHC.KindedTyVar _ (GHC.dL->(sspan , name)) _))) ->
            pure (sspan, RtkBind name)
        _ -> empty
...
decls (group, _, _, _) = concatMap ($ group)
  ...
  where
    ...
    fun term = case cast term of
        ...
        (Just (GHC.PatSynBind _ (GHC.PSB _ (GHC.dL->(sspan , name)) _ _ _)))
            | GHC.isExternalName name -> pure (sspan, RtkDecl name)
        _ -> empty
    ...
    fix term = case cast term of
        Just ((GHC.FixitySig _ names _) :: GHC.FixitySig GHC.GhcRn)
          -> map (\(GHC.dL->(sspan , x)) -> (sspan, RtkVar x)) names
        Just ((GHC.XFixitySig {}) :: GHC.FixitySig GHC.GhcRn)
          -> GHC.panic "haddock:decls"
        Nothing -> empty
    tyfam (GHC.dL->(_ , GHC.FamilyDecl{..})) = [decl fdLName]
    tyfam _                                  = GHC.panic "haddock:dels"
    sig (GHC.dL->(_ , GHC.TypeSig _ names _)) = map decl names
    sig (GHC.dL->(_ , GHC.PatSynSig _ names _)) = map decl names
    sig (GHC.dL->(_ , GHC.ClassOpSig _ _ names _)) = map decl names
    sig _ = []
    decl (GHC.dL->(sspan , name)) = (sspan, RtkDecl name)
    tyref (GHC.dL->(sspan , name)) = (sspan, RtkType name)
...
imports src@(_, imps, _, _) =
  ...
  where
    ie term = case cast term of
        ...
        _ -> empty
    typ (GHC.dL->(sspan , name)) = (sspan, RtkType name)
    var (GHC.dL->(sspan , name)) = (sspan, RtkVar name)
    modu (GHC.dL->(sspan , name)) = (sspan, RtkModule name)
    imp idecl
