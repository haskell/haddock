...
linkedAnchor :: Text -> [Attributes] -> Html () -> Html ()
linkedAnchor n attrs = a_ (href_ (Text.cons '#' n) : attrs)
...
collapseToggle id' classes = [class_ cs, makeAttributes "data-details-id" id']
 where
  cs = Text.unwords (Text.words classes ++ ["details-toggle"])
...
collapseControl id' classes = collapseToggle id' cs
 where
  cs = Text.unwords (Text.words classes ++ ["details-toggle-control"])
