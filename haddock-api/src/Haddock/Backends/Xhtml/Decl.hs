...
ppSigLike
  ...
  emptyCtxts =
    ppTypeOrFunSig
      ...
      , (leader <+>) . addFixities . mconcat . punctuate comma $ map (ppBinder False) occnames
      , dcolon unicode
      )
      splice
      unicode
    ...
    | otherwise =
        topDeclElem
          ...
          <> subArguments
            pkg
            qual
            (ppSubSigLike unicode qual typ argDocs [] sep emptyCtxts)
          <> docSection
            curname
            pkg
            qual
...
ppSubSigLike unicode qual typ argDocs subdocs sep emptyCtxts = do_sig_args 0 sep typ
 where
  ...
  gadtComma = mconcat (replicate (if unicode then 2 else 3) (toHtmlRaw ("&nbsp;" :: Text))) <> toHtml ("," :: Text)
  gadtEnd = mconcat (replicate (if unicode then 2 else 3) (toHtmlRaw ("&nbsp;" :: Text))) <> toHtml ("}" :: Text)
  gadtOpen = toHtml ("{" :: Text)

ppFixities :: [(DocName, Fixity)] -> Qualification -> Html ()
ppFixities [] _ = pure ()
ppFixities fs qual = foldr1 (<>) (map ppFix uniq_fs) <> rightEdge
 where
  ppFix (ns, p, d) = (span_ [class_ "fixity"]) (toHtml d <+> toHtml (show p) <+> ppNames ns)
  ...
  ppNames = case fs of
    _ : [] -> const (pure ()) -- Don't display names for fixities on single names
    _ -> mconcat . intersperse (toHtml (", " :: Text)) . map (ppDocName qual Infix False)
  ...
  rightEdge = (span_ [class_ "rightedge"]) (pure ())
...
ppFor
  ...
  pkg
  qual =
    ppFunSig summary links loc (pure ()) doc [name] typ fixities splice unicode pkg qual
...
ppTySyn
  ...
  qual =
    ppTypeOrFunSig
      ...
      sig_type
      doc
      (full <+> fixs, hdr <+> fixs, toHtmlRaw ("&nbsp;" :: Text) <> equals)
      splice
      unicode
      pkg
      qual
...
ppTypeSig summary nms pp_ty unicode =
  mconcat htmlNames <+> dcolon unicode <+> pp_ty
 where
  htmlNames = intersperse (toHtml (", " :: Text)) $ map (ppBinder summary) nms
...
ppFamDecl summary associated links instances fixities loc doc decl splice unicode pkg qual
  ...
  | otherwise = header' <> docSection curname pkg qual doc <> instancesBit
 where
...
ppContextNoLocsMaybe [] _ _ emptyCtxts =
  case emptyCtxts of
    ...
    ShowEmptyToplevelContexts -> Just (toHtml ("()" :: Text))
...
ppClassHdr summ lctxt n tvs fds unicode qual =
  keyword "class"
    <+> (if not (null $ fromMaybeContext lctxt) then ppLContext lctxt unicode qual HideEmptyContexts else pure ())
  ...
  qual =
    if not (any isUserLSig sigs) && null ats
      then (if summary then id else topDeclElem links loc splice [nm]) hdr
        ...
        (if summary then id else topDeclElem links loc splice [nm])
          (hdr <+> keyword "where")
          <> shortSubDecls
            False
  ...
  qual
   ...
   where
    ...
    minimalBit = case [s | MinimalSig _ (L _ s) <- sigs] of
      ...
      And [] : _ -> subMinimal $ toHtml ("Nothing" :: Text)
...
ppInstHead links splice unicode qual mdoc origin orphan no ihd@(InstHead{..}) mdl =
 ...
 where
  mname = maybe (pure ()) (\m -> toHtml ("Defined in" :: Text) <+> ppModule m) mdl
...
instanceId :: InstOrigin DocName -> Int -> Bool -> InstHead DocNameI -> Text
instanceId origin no orphan ihd =
  mconcat $
    ["o:" | orphan]
      ++ [ qual origin
         , ":" <> Text.pack (getOccString origin)
         , ":" <> Text.pack (getOccString (ihdClsName ihd))
         , ":" <> Text.pack (show no)
         ]
 where
...
ppShortDataDecl summary dataInst dataDecl pats unicode qual
  ...
  , isH98 =
      dataHeader
        <> shortSubDecls
          dataInst
          (zipWith doConstr ('=' : repeat '|') (toList cons) ++ pats1)
  | otherwise =
      (dataHeader <+> keyword "where")
        <> shortSubDecls
          dataInst
          (map doGADTConstr (toList cons) ++ pats1)
 where
  dataHeader
    | dataInst = pure ()
...
ppDataDecl
  ...
  qual
    ...
    | otherwise = header' <> docSection curname pkg qual doc <> constrBit <> patternBit <> instancesBit
   where
    ...
    ConDeclH98
      ...
      , con_mb_cxt = cxt
        ...
        let context = fromMaybeContext cxt
            header' = ppConstrHdr forall_ tyVars context unicode qual
         in case det of
              -- Prefix constructor, e.g. 'Just a'
              PrefixCon _ args ->
                ( header' <+> hsep (ppOcc : map (ppLParendType unicode qual HideEmptyContexts . hsScaledThing) args)
                , pure ()
                , pure ()
                )
      ...
      , con_mb_cxt = cxt
        ...
        let context = fromMaybeContext cxt
         ...
         in case det of
              -- Prefix constructor, e.g. 'Just a'
              PrefixCon _ args
                | hasArgDocs -> header' <+> ppOcc <+> fixity
                | otherwise ->
                    hsep
                      [ header' <+> ppOcc
              ...
              -- Record constructor, e.g. 'Identity { runIdentity :: a }'
              RecCon _ -> header' <+> ppOcc <+> fixity
              -- Infix constructor, e.g. 'a :| [a]'
              InfixCon arg1 arg2
                | hasArgDocs -> header' <+> ppOcc <+> fixity
                | otherwise ->
                    hsep
                      [ header' <+> ppLParendType unicode qual HideEmptyContexts (hsScaledThing arg1)
                      , ppOccInfix
                      , ppLParendType unicode qual HideEmptyContexts (hsScaledThing arg2)
                      , fixity
                      ]
...
ppBang (HsSrcBang _ _ SrcStrict) = toHtml ("!" :: Text)
ppBang (HsSrcBang _ _ SrcLazy) = toHtml ("~" :: Text)
ppBang _ = pure ()
...
ppLHsTypeArg unicode qual emptyCtxts (HsTypeArg _ ki) =
  atSign unicode
    <> ppLParendType unicode qual emptyCtxts ki
ppLHsTypeArg _ _ _ (HsArgPar _) = toHtml ("" :: Text)
...
ppForAllPart unicode qual tele = case tele of
  HsForAllVis{hsf_vis_bndrs = bndrs} ->
    hsep
      (forallSymbol unicode : ppTyVars unicode qual bndrs)
      <> (toHtmlRaw ("&nbsp;" :: Text))
      <> arrow
        unicode
...
ppr_mono_ty (HsTyVar _ _ (L _ name)) True _ _
  | getOccString (getName name) == "(->)" = toHtml ("(→)" :: Text)
...
ppr_mono_ty (HsStarTy _ isUni) u _ _ =
  toHtml (if u || isUni then "★" else "*" :: Text)
...
ppr_mono_ty (HsRecTy{}) _ _ _ = toHtml ("{..}" :: Text)
